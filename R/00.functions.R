dexseq_group <- function(object, index, stage) {
  object <- object[, index]
  res <- DEXSeq(object,
...
get_pathway <- function(res, group, pathway, kegg) {
  p <- list()
  res <- res %>%
    filter(stage == group)
  p$pvalues <- res$pvalue
  p$index <- which(unlist(res$row %in% kegg[[pathway]]))
  p$weights <- res[p$index, 'stat'] %>% unlist
  p
}

melt_dds <- function(dds,
                     mat = NULL,
                     col_data = NULL) {
  mat <- assay(dds)
  col_data <- colData(dds) %>% as.data.frame %>% mutate(sample = rownames(colData(dds)))
  mat %>%
    melt %>%
...
modified_vennDiagram <- function (object, include = "both", names = NULL, mar = rep(1, 
